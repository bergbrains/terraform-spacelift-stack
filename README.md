...
| <a name="input_enable_sensitive_outputs_upload"></a> [enable\_sensitive\_outputs\_upload](#input\_enable\_sensitive\_outputs\_upload) | Indicates whether sensitive outputs created by this stack can be uploaded to Spacelift to be used by Stack Dependency references. | `bool` | `true` | no |
...
| <a name="input_spacelift_api_key_endpoint"></a> [spacelift\_api\_key\_endpoint](#input\_spacelift\_api\_key\_endpoint) | The Spacelift API endpoint URL. Defaults to https://<spacelift\_account\_name>.app.spacelift.io. Can also be set via the SPACELIFT\_API\_KEY\_ENDPOINT environment variable. | `string` | `null` | no |
...
| <a name="input_terraform_smart_sanitization"></a> [terraform\_smart\_sanitization](#input\_terraform\_smart\_sanitization) | Indicates whether runs will use Terraform's sensitive value system to sanitize outputs of Terraform state and plans in Spacelift instead of sanitizing all fields. Requires Terraform >= 1.0.1. | `bool` | `false` | no |
